# -*- coding: utf-8 -*-
"""
Script 4-1 — Árvore de Decisão (Decision Tree)
...
def plot_feature_importance(model, feature_names, branch_label):
    ...
    ax.bar(range(len(top_idx)), importances[top_idx], color="#4C72B0")
...
def main():
    ...
    model_p = DecisionTreeClassifier(class_weight="balanced", random_state=RANDOM_STATE)
    model_p.fit(Xp_tr, yp_tr)

    print("Treinando Branch Feira...")
    model_f = DecisionTreeClassifier(class_weight="balanced", random_state=RANDOM_STATE)
